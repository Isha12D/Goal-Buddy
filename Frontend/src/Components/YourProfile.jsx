...
const YourProfile = () => {
  ...
  const fetchTopGoals = async () => {
    try {
      const response = await axios.get(`http://localhost:3006/top-completed-goals/${currentUser?._id}`);
  ...
  const fetchTopIncompleteGoals = async () => {
    try {
      const response = await axios.get(`http://localhost:3006/top-incomplete-goals/${currentUser?._id}`);
